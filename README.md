...
Job Flow:
1. **docker-build-and-push**: Multi-stage build, push to `registry.toolz.fullstack.pw/library/cks-frontend:latest` and commit SHA
...
containers:
  - name: cks-frontend
    image: registry.toolz.fullstack.pw/library/cks-frontend:latest
    ports:
      - containerPort: 3000
    envFrom:
      - configMapRef:
